...
def save_to_csv(data, write_header=False):
    ...
    with open(CSV_PATH, "a", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["title", "meta_image", "content", "url", "label"]
        )
...
async def fetch_article_content(context, url, retries=3, delay=2):
    for attempt in range(1, retries + 1):
        ...
        try:
            ...
            await page.wait_for_selector("article.col-12.mb-article-details.articles-observe", timeout=8000)

            html = await page.content()
            soup = BeautifulSoup(html, "html.parser")

            paragraphs = soup.select(".article-full-body div.article-text, .article-full-body p") 
...
async def click_older_posts_button(page, max_retries=3):
    selectors = ['a[rel="next"]', 'a[aria-label="Next »"]']
...
async def extract_article_links(page):
    await page.wait_for_selector("div.sw-grid-a.pt-4.mb-3, div.sw-audio-a.pt-4", timeout=8000)
    
    html = await page.content()
    soup = BeautifulSoup(html, "html.parser")
    links = []
    article_selectors = [
        ".sw-audio-a a.mb-black", 
        ".mb-latest-news-list .desc a.mb-black"
    ]

    for selector in article_selectors:
        articles = soup.select(selector)
        print(f"Found {len(articles)} potential links with selector: {selector}")
        
        for article in articles:
            href = article.get("href")
            if href:
                if href.startswith('/'):
                    href = "https://mb.com.ph" + href
...
# -------- main scraper --------
async def scrape():
    start_url = "https://mb.com.ph/category/sports?page=5"
    batch_size = 10
    target_total = 2000
    scraped_links = set()
    failed_links = set()
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            selectors = [
                "div.sw-audio-a.pt-4.entertainment-section a",
                "div.sw-grid-a.pt-4.mb-3 a"
            ]
            for selector in selectors:
                try:
